...
module.exports = class BaselineManagementApi extends ConfigurationManagementApi {
  ...
  async getBaselines(filter) {
    try {
      const baselines = await this.safeDot4ClientRequest(
        'get',
        `/api/baseline/dropdown`,
      );
      if (filter) {
    ...
    try {
      let result = _.get(
        await this.safeDot4ClientRequest(
          'get',
          `/api/Baseline/BaselineCIs/${baselineId}` +
            (ciId ? '?$filter=' + querystring.escape('ciId eq ' + ciId) : ''),
        ),
        'items',
      );
      // debug(result)
  ...
  async execBaselinesForCi(ci_or_ciid, baselineId) {
    try {
      if (!this.baselines) this.baselines = await this.getBaselines();
