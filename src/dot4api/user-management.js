...
module.exports = class UserManagementApi extends ConfigurationManagementApi {
  ...
  async updatePerson(oldUser, newUser) {
    ...
    _.forEach(oldPerson, (v, k_orig) => {
      ...
      if (!_.has(newUser, k_orig) && !_.has(newUser, k_other)) {
        //hier muss newUser genommen werden, da newPerson Default-Werte gesetzt hat
    ...
    _.forEach(newPerson, (v, k) => {
      if (!_.has(oldPerson, k)) delete newPerson[k];
    });
  ...
  async insertPerson(dot4user) {
    ...
    if (!this.asyncInitialisationsFinished) await this.asyncInitialisationsP;
    ...
    if (
      !_.has(dot4user, 'userId_PERS') &&
      !_.has(dot4user, 'userExisting_PERS')
    ) {
  ...
  async assignDefaultRoles(userid) {
    ...
    let portalRole =
      this.roles['B827D132-2866-40BB-B114-1520B3A72FFF'] ||
      _.find(this.roles, (roleObj) => roleObj.name.toLowerCase() == 'portal');
  ...
  async assignRolesforUser(userid, roles) {
    ...
    await this.safeDot4ClientRequest(
      'put',
  ...
  async loadAllUsers() {
    debug('loadAllUsers()');
    return await this.loadAllCisForFilter(
      `ciTypeId eq ${Person.getCiTypeAttribute(this.ciTypes, 'id')}`,
      {
        ciTypeName: Person.getCiTypeAttribute(this.ciTypes, 'name'),
      },
    );
  }
  ...
  async loadDepartment(departmentName, companyID) {
    ...
    let departments = await this.loadAllCisForFilter(
      `ciTypeId eq ${Department.getCiTypeAttribute(this.ciTypes, 'id')}`,
      {
    ...
    for (let dep of departments) {
      let companyRelation = _.get(
        await this.getRelationsForCI(
          dep.id,
          "(relationTypeUuid eq '575689b9-4851-429e-8785-5f6ee3dacb1b')",
        ),
        'items[0]',
      );
  ...
  async loadAllCompanies() {
    ...
    return await this.loadAllCisForFilter(
      `ciTypeId eq ${Company.getCiTypeAttribute(this.ciTypes, 'id')}`,
      {
        ciTypeName: Company.getCiTypeAttribute(this.ciTypes, 'name'),
      },
    );
  }
};
