...
const UUID_LIFECYCLE_STATUS_OPERATIONAL =
  '9eeb3f18-1b55-4f8f-a63a-262a41325e20';

const UUID_RELATIONTYPE_SERVICE_TO_SERVICE =
  'cfd33042-c1ae-4e7b-b654-7deaa622fc2e';
const UUID_RELATIONTYPE_SERVICE_TO_TECHNICAL_SERVICE =
  '9274c1e8-b599-4f9a-ab37-55d32fb8596f';
...
class ServiceManagementApi extends ConfigurationManagementApi {
  ...
  async getServicesDropdown() {
    try {
      ...
      const response = await this.getCisByCiTypeUuidDropdown(
        UUID_CITYPE_SERVICE,
      );
  ...
  async createTechnicalService(technicalService) {
    try {
      debug(
        `${this.name}.createTechnicalService("${JSON.stringify(
          technicalService,
        )}") ...`,
      );

      return await this.createCi(
        technicalService,
        UUID_CITYPE_TECHNICAL_SERVICE,
      );
    } catch (error) {
  ...
  async createServiceRelation(sourceId, destinationIds) {
    try {
      debug(
        `${this.name}.createServiceRelation(${sourceId}, ${destinationIds}) ...`,
      );

      return await this.createRelation(
        sourceId,
        destinationIds,
        UUID_RELATIONTYPE_SERVICE_TO_SERVICE,
      );
    } catch (error) {
      throw error;
    } finally {
  ...
  async createTechnicalServiceRelation(sourceId, destinationIds) {
    try {
      debug(
        `${this.name}.createTechnicalServiceRelation(${sourceId}, ${destinationIds}) ...`,
      );

      return await this.createRelation(
        sourceId,
        destinationIds,
        UUID_RELATIONTYPE_SERVICE_TO_TECHNICAL_SERVICE,
      );
    } catch (error) {
      throw error;
    } finally {
